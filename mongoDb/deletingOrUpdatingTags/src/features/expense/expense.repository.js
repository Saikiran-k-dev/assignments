...
class ExpenseRepository {
  ...
  async updateTagInExpense(id, oldTag, newTag) { 
    ...
    // console.log(id)
    const tagExist = await collection.findOne({_id: new ObjectId(id)})
    
    // console.log(tagExist)
    if(tagExist){
      
      const findIndexOfTag = tagExist.tags.indexOf(oldTag)
      
      console.log(findIndexOfTag)
      if(findIndexOfTag>=0){
        const tags = tagExist.tags
        tags[findIndexOfTag] = newTag
        console.log(tags)
        await collection.updateOne({_id: new ObjectId(id)},{$unset:{"tags":""}})
        await collection.updateOne({_id: new ObjectId(id)},{$push:{"tags":tags}})
